...
function useFileUpload(options: UploadOptions): {
  ...
  const uploadFile = async (file: File): Promise<string | null> => {
    if (file.size > options.maxSize) {
      const error = new Error('File size exceeds 5MB limit. Please select a smaller image.')
    ...
    try {
      ...
      // Get the image type from the file

      // Shorten the path by removing the data URL prefix
      const shortenedUrl = url.replace(/^data:image\/[^;]+;base64,/, '')
      ...
      if (!abortController.signal.aborted) {
        ...
        options.onSuccess?.(shortenedUrl)
        return shortenedUrl
      }
...
export const ImageUploadNode: React.FC<NodeViewProps> = (props) => {
  ...
  const handleUpload = async (files: File[]): Promise<void> => {
    ...
    if (url) {
      ...
      // If the url is a base64 string, reconstruct the data URL with the correct type and name
      // Otherwise, just use the url as src
      let src: string
      if (/^[A-Za-z0-9+/=]+$/.test(url) && url.length > 100) {
        // Looks like a base64 string, reconstruct data URL
        src = `data:${imageType};name=${encodeURIComponent(filename)};base64,${url}`
      } else {
        // Otherwise, use the url as is (could be a file path or remote url)
        src = url
      }
    ...
    <NodeViewWrapper className="tiptap-image-upload" tabIndex={0} onClick={handleClick}>
      {!fileItem && (
        <ImageUploadDragArea onFile={handleUpload}>
          <DropZoneContent maxSize={5 * 1024 * 1024} />
        </ImageUploadDragArea>
      )}
